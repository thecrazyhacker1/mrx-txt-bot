...
async def text_to_txt(client, message: Message):
    ...
    editable = try:
        await message.reply_text(f"<blockquote><b>Welcome to the Text to .txt Converter!\nSend the **text** for convert into a `.txt` file.</b></blockquote>")
    except FloodWait as fw:
        await asyncio.sleep(fw.value)
        await message.reply_text(f"<blockquote><b>Welcome to the Text to .txt Converter!\nSend the **text** for convert into a `.txt` file.</b></blockquote>")
    ...
    try:
        ...
        await m.reply_text("🚦**STOPPED**🚦")
            ...
            if "youtube.com" in url or "youtu.be" in url:
                prog = try:
        await m.reply_text(f"<i><b>Audio Downloading</b></i>\n<blockquote><b>{str(count)
    except FloodWait as fw:
        await asyncio.sleep(fw.value)
...
async def vidwatermark_handler(client: Client, m: Message):
    global vidwatermark
    editable = try:
        await m.reply_text("**Send Video Watermark text, else Send /d**")
    except FloodWait as fw:
        await asyncio.sleep(fw.value)
        await m.reply_text("**Send Video Watermark text, else Send /d**")
...
async def topic_handler(client: Client, m: Message):
    global topic
    editable = try:
        await m.reply_text("**If you want to topic wise uploader : send `yes` or send /d**\n\n<blockquote><b>Topic fetch from (bracket)
    except FloodWait as fw:
        await asyncio.sleep(fw.value)
...
async def token_handler(client: Client, m: Message):
    ...
    editable = try:
        await m.reply_text("<b>Enter 𝐏𝐖/𝐂𝐖/𝐂𝐏 Working Token For 𝐌𝐏𝐃 𝐔𝐑𝐋 or send /d</b>")
    except FloodWait as fw:
        await asyncio.sleep(fw.value)
        await m.reply_text("<b>Enter 𝐏𝐖/𝐂𝐖/𝐂𝐏 Working Token For 𝐌𝐏𝐃 𝐔𝐑𝐋 or send /d</b>")
    input: Message = await bot.listen(editable.chat.id)
...
async def send_logs(client: Client, m: Message):  # Correct parameter name
    try:
        with open("logs.txt", "rb") as file:
            sent = try:
        await m.reply_text("**📤 Sending you ....**")
    except FloodWait as fw:
        await asyncio.sleep(fw.value)
        await m.reply_text("**📤 Sending you ....**")
...
async def txt_handler(bot: Client, m: Message):  
    ...
    if m.chat.id not in AUTH_USERS:
        ...
        await bot.send_message(m.chat.id, f"<blockquote>__**Oopss! You are not a Premium member\nPLEASE /upgrade YOUR PLAN\nSend me your user id for authorization\nYour User id**__ - `{m.chat.id}`</blockquote>\n")
            return
    editable = try:
        await m.reply_text(f"**__Hii, I am drm Downloader Bot__\n<blockquote><i>Send Me Your text file which enclude Name with url...\nE.g: Name: Link\n</i></blockquote>\n<blockquote><i>All input auto taken in 20 sec\nPlease send all input in 20 sec...\n</i></blockquote>**")
    except FloodWait as fw:
        await asyncio.sleep(fw.value)
        await m.reply_text(f"**__Hii, I am drm Downloader Bot__\n<blockquote><i>Send Me Your text file which enclude Name with url...\nE.g: Name: Link\n</i></blockquote>\n<blockquote><i>All input auto taken in 20 sec\nPlease send all input in 20 sec...\n</i></blockquote>**")
    input: Message = await bot.listen(editable.chat.id)
    ...
    try:
        if raw_text == "1":
            batch_message = try:
        await bot.send_message(chat_id=channel_id, text=f"<blockquote><b>🎯Target Batch : {b_name}</b></blockquote>")
    except FloodWait as fw:
        await asyncio.sleep(fw.value)
        await bot.send_message(chat_id=channel_id, text=f"<blockquote><b>🎯Target Batch : {b_name}</b></blockquote>")
    ...
    try:
        ...
        await m.reply_text(str(e))
                    ...
                    prog = try:
        await bot.send_message(channel_id, Show, disable_web_page_preview=True)
    except FloodWait as fw:
        await asyncio.sleep(fw.value)
        await bot.send_message(channel_id, Show, disable_web_page_preview=True)
                    prog1 = try:
        await m.reply_text(Show1, disable_web_page_preview=True)
    except FloodWait as fw:
        await asyncio.sleep(fw.value)
        await m.reply_text(Show1, disable_web_page_preview=True)
...
async def text_handler(bot: Client, m: Message):
    ...
    editable = try:
        await m.reply_text(f"<pre><code>**🔹Processing your link...\n🔁Please wait...⏳**</code></pre>")
    except FloodWait as fw:
        await asyncio.sleep(fw.value)
        await m.reply_text(f"<pre><code>**🔹Processing your link...\n🔁Please wait...⏳**</code></pre>")
    await m.delete()
    ...
    try:
        ...
        await m.reply_text(str(e))
                    ...
                    Show = f"**⚡Dᴏᴡɴʟᴏᴀᴅɪɴɢ Sᴛᴀʀᴛᴇᴅ...⏳**\n" \
                           ...
                           f"✦𝐁𝐨𝐭 𝐌𝐚𝐝𝐞 𝐁𝐲 ✦ {CREDIT}"
                    prog = try:
        await m.reply_text(Show, disable_web_page_preview=True)
    except FloodWait as fw:
        await asyncio.sleep(fw.value)
        await m.reply_text(Show, disable_web_page_preview=True)
                    res_file = await helper.download_and_decrypt_video(url, cmd, name, appxkey)  
